...
export class SecondStrategy {
    ...
    public addProcess(p: Process): void {
        ...
        this.queries++;
        if (x.getLoad < this.valueOfP)
            x.addProcess(newProcess);
        else {
            let id: number;
            while ((id = ProcessorManager.getRandomInt(0, this.processors.length)) == processorId && this.processors[id].getLoad >= this.valueOfP)
                this.queries++;
